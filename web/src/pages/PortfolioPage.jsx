...
function PositionCard({ pos, idx, onClose, onAIConsult }) {
  ...
  return (
    <motion.div
      ...
      className="group relative overflow-hidden rounded-3xl border border-white/5 bg-neutral-900/40 p-6 backdrop-blur-xl"
    >
      {/* Chrome */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/12 to-transparent" />
      <div className={`pointer-events-none absolute inset-y-0 left-0 w-[3px] rounded-r-full transition-opacity ${isPnlPos ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.6)]' : 'bg-rose-400 shadow-[0_0_8px_rgba(248,113,113,0.6)]'}`} />

      {/* Header */}
      <div className="mb-5 flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2.5">
            <h3 className="text-2xl font-black tracking-tight text-neutral-50">
        ...
        <div className={`rounded-2xl border px-4 py-2.5 text-right ${
          ...
          <p className={`text-2xl font-black tabular-nums leading-none ${isPnlPos ? 'text-emerald-400' : 'text-rose-400'}`}>
            {isPnlPos ? '+' : ''}{pos.pnl.toFixed(2)}
          </p>
          <p className={`mt-0.5 text-[11px] font-bold ${isPnlPos ? 'text-emerald-600' : 'text-rose-600'}`}>
      ...
      <div className="mb-5 grid grid-cols-3 gap-3">
        {[
          ...
          <div key={label} className="rounded-2xl bg-neutral-950/60 p-3">
            <p className="mb-1 text-[9px] uppercase tracking-widest text-neutral-600">{label}</p>
            <p className="text-sm font-bold tabular-nums text-neutral-200">{value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-3">
        <motion.button
          ...
          className="flex flex-1 items-center justify-center gap-2 rounded-2xl border border-violet-500/25 bg-gradient-to-r from-violet-500/10 to-cyan-500/10 px-4 py-2.5 text-sm font-semibold text-violet-300 transition-all hover:border-violet-400/40 hover:from-violet-500/18 hover:to-cyan-500/18 hover:text-violet-200"
        >
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 pb-24">
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-3xl border border-white/5 bg-neutral-900/40 p-8 backdrop-blur-xl"
      >
        {/* Background chrome */}
        ...
        <div className="relative">
          <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.35em] text-neutral-600">
            Total Portfolio PnL
          </p>
          ...
          >
            <span className={`text-7xl font-black tabular-nums tracking-tight leading-none sm:text-8xl ${
              isPnlPos
            ...
            <span className="ml-3 text-2xl font-bold text-neutral-600">USDT</span>
          </motion.div>

          {/* Mini stat pills */}
          <div className="mt-6 flex flex-wrap gap-3">
            {[
              { label: 'Open',     value: openPositions.length, color: 'text-neutral-200' },
              { label: 'Winning',  value: winCount,             color: 'text-emerald-400' },
              { label: 'Losing',   value: lossCount,            color: 'text-rose-400'    },
              ...(summaryData?.stats?.total_trades !== undefined
                ? [{ label: 'Lifetime Trades', value: summaryData.stats.total_trades, color: 'text-cyan-400' }]
                : []),
            ].map(({ label, value, color }) => (
              <div key={label} className="rounded-2xl border border-white/5 bg-neutral-950/50 px-4 py-2">
        ...
        <AnimatePresence mode="popLayout">
          {openPositions.length > 0 ? (
            ...
            >
              ...
              <p className="text-sm font-bold text-neutral-500">No open positions.</p>
              <p className="mt-1 text-xs text-neutral-700">Execute a trade from the Terminal to begin.</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
